...
def search_by_token_limit(
    ...
    for res in candidate_results:
        ...
        if not document_text:
            continue
        encoded_tokens = tokenizer.encode(document_text)
        ...
        if num_tokens <= remaining_space:
            ...
            truncated_tokens = encoded_tokens[:remaining_space]
...
def build_prompt(pr_data, choose_BASE, choose_ID, choose_OD, experiment_mode, top):
    ...
    tips = "To ensure valid JSON output, always enclose all keys and string values in double quotes, and ensure every element within a list(if the string values is a list) is also correctly quoted."
    system_prompt += tips
